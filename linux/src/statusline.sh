#!/usr/bin/env bash
# Claude Tap - Status line script for Claude Code (Linux).
#
...
# Rate limit warnings are triggered here (via the notification overlay)
...
format_reset() {
  ...
  if [ "$days" -gt 0 ]; then
    ...
    clock=$(date -d "@${resets_sec}" +"%a %H:%M")
  else
    duration="${hours}h${mins}m"
    clock=$(date -d "@${resets_sec}" +"%H:%M")
  fi
  echo "${duration} · ${clock}"
}

# Helper: play a sound using the best available method
play_sound() {
    local file="$1" volume="$2"
    if [ ! -f "$file" ]; then return; fi
    if command -v paplay &>/dev/null; then
        local pa_vol
        pa_vol=$(python3 -c "print(int(float('$volume') * 65536))" 2>/dev/null || echo "9830")
...
# Helper: show a notification using the best available method
show_notification() {
    local title="$1" message="$2" icon="$3" urgency="$4"
    local notification_py="$CONFIG_DIR/notification.py"
    if [ -f "$notification_py" ]; then
        python3 "$notification_py" "$title" "$message" "$icon" "$urgency" &
...
if [ "$SHOW_5H" = "true" ] && [ -n "$five_hour" ]; then
  ...
  if [ "$rate_int" -ge "$CRIT_THRESHOLD" ]; then
    if [ ! -f "$CLAUDE_TMPDIR/claude-rate-warn-critical" ]; then
      ...
      show_notification "Rate Limit Warning" "5h usage at ${rate_int}%.${reset_msg} Consider slowing down." "$CONFIG_DIR/claude-icon.png" "critical"
      if [ "$SND_ENABLED" = "true" ]; then
          play_sound "${SND_RATE_WARN:-$SND_DEFAULT}" "$SND_VOLUME"
      fi
    fi
    ...
    if [ ! -f "$CLAUDE_TMPDIR/claude-rate-warn-warning" ]; then
      ...
      reset_msg=""
      [ -n "$reset_str" ] && reset_msg=" Resets in ${reset_str}."
      show_notification "Rate Limit Warning" "5h usage at ${rate_int}%.${reset_msg}" "$CONFIG_DIR/claude-icon.png" "warning"
      if [ "$SND_ENABLED" = "true" ]; then
          play_sound "${SND_RATE_WARN:-$SND_DEFAULT}" "$SND_VOLUME"
      fi
    fi

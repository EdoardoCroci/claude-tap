#!/bin/bash
# Claude Tap - Installer (Linux)
#
# This script:
#   1. Checks prerequisites (Linux, python3, jq, GTK3)
#   2. Asks you to configure notifications, status line, and colors
#   3. Copies assets (icon, sound, notification.py) to ~/.config/claude-tap/
#   4. Generates config.json from your choices
#   5. Registers Claude Code hooks and status line in ~/.claude/settings.json
#
# Safe to run multiple times. Pass --reconfigure to re-run the interactive
# setup even if a config already exists.
...
echo ""
echo -e "${BOLD}Claude Tap - Installer (Linux)${RESET}"
echo ""
...
[ "$(uname -s)" = "Linux" ] || fail "This installer is for Linux only. Use macos/install.sh on macOS."

if command -v python3 &>/dev/null; then
    success "python3 found"
else
    fail "python3 not found. Install it via your package manager (e.g., apt install python3)."
fi

# Check for GTK3 (optional - falls back to notify-send)
HAS_GTK="false"
if python3 -c "import gi; gi.require_version('Gtk', '3.0'); from gi.repository import Gtk" 2>/dev/null; then
    success "GTK3 (PyGObject) found - custom overlay enabled"
    HAS_GTK="true"
else
    warn "GTK3 (PyGObject) not found - will use notify-send as fallback"
    warn "  Install for custom overlay: apt install python3-gi gir1.2-gtk-3.0 (Debian/Ubuntu)"
    warn "                              dnf install python3-gobject gtk3 (Fedora)"
    warn "                              pacman -S python-gobject gtk3 (Arch)"
    if command -v notify-send &>/dev/null; then
        success "notify-send found (fallback)"
    else
        warn "notify-send not found either. Notifications will be silent."
    fi
fi
...
if command -v jq &>/dev/null; then
    ...
    warn "jq not found. The status line requires jq. Install via: apt install jq / dnf install jq / pacman -S jq"
fi

# Check for sound tools
SOUND_TOOL="none"
if command -v paplay &>/dev/null; then
    SOUND_TOOL="paplay"
    success "paplay found (PulseAudio)"
elif command -v pw-play &>/dev/null; then
    SOUND_TOOL="pw-play"
    success "pw-play found (PipeWire)"
elif command -v aplay &>/dev/null; then
    SOUND_TOOL="aplay"
    success "aplay found (ALSA)"
else
    warn "No sound tool found (paplay, pw-play, aplay). Sound will be disabled."
fi

echo ""

# ──────────────────────────────────────────────────────────────
# 1b. Check for updates
# ──────────────────────────────────────────────────────────────
...
" 2>/dev/null)" || true
    ...
    if [ "$SOUND_TOOL" = "none" ]; then
        echo -e "    ${DIM}No sound tool detected. Sound will be disabled.${RESET}"
        SOUND_ENABLED="false"
        ...
        if [ "$SOUND_ENABLED" = "true" ]; then
            ...
            while [ "$PREVIEW_LOOP" = "true" ]; do
                ...
                if [ "$SOUND_TOOL" = "paplay" ]; then
                    PA_VOL=$(python3 -c "print(int(float('$SOUND_VOLUME') * 65536))" 2>/dev/null || echo "9830")
                    paplay --volume="$PA_VOL" "$RESOLVED_SOUND" 2>/dev/null
                elif [ "$SOUND_TOOL" = "pw-play" ]; then
                    pw-play --volume="$SOUND_VOLUME" "$RESOLVED_SOUND" 2>/dev/null
                elif [ "$SOUND_TOOL" = "aplay" ]; then
                    aplay -q "$RESOLVED_SOUND" 2>/dev/null
                fi
...
cp "$SCRIPT_DIR/src/notification.py" "$CONFIG_DIR/notification.py"
chmod 0600 "$CONFIG_DIR/claude-icon.png" "$CONFIG_DIR/default.wav" "$CONFIG_DIR/themes.json"
chmod 0700 "$CONFIG_DIR/notification.py"
success "Assets copied (icon + sound + themes + notification overlay)"
...
echo -e "  Overlay:    ${CYAN}$CONFIG_DIR/notification.py${RESET}"
echo -e "  Hooks:      ${CYAN}$SETTINGS_FILE${RESET}"
if [ "$HAS_GTK" = "false" ]; then
    echo ""
    echo -e "  ${YELLOW}Note:${RESET} Using notify-send fallback. Install GTK3 for the custom overlay."
fi
echo ""
...
echo -e "     Changes take effect immediately."
echo -e "  4. Run ${CYAN}./install.sh --reconfigure${RESET} to re-run this setup wizard"
echo ""

# Show a test notification
if [ -f "$CONFIG_DIR/notification.py" ] && [ "$HAS_GTK" = "true" ]; then
    python3 "$CONFIG_DIR/notification.py" "Claude Tap" "Installation successful! Click to focus terminal." "$CONFIG_DIR/claude-icon.png" &
elif command -v notify-send &>/dev/null; then
    notify-send "Claude Tap" "Installation successful!" --icon="$CONFIG_DIR/claude-icon.png" &
fi
